...
export default function WeddingCalendar({ date }) {
  ...
  return (
    <div className="inline-block rounded-md border border-[#e5bc6f] p-4 bg-white">
      {/* tiêu đề thứ */}
      ...
      <div className="mt-2 grid grid-cols-7 gap-2 text-center text-lg font-medium">
        {days.map((d, i) => {
          ...
          return (
            <div
              key={i}
              className="relative h-6 w-11 flex items-center justify-center"
            >
              {d ? (
                isSelected ? (
                  <>
                    <svg
                      viewBox="0 0 24 24"
                      className="absolute h-10 w-10 text-red-500"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="1.5"
                    >
                    ...
                    <span className="relative z-10 text-red-600 font-bold">
                      {d}
                    </span>
                  </>
                ) : (
